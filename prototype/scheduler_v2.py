...
class SchedulerV2:
    ...
    def create_irrigation_jobs(self, irrigation_schedule: List[datetime]):
        irrigation_jobs = [self.scheduler.add_job(actuator_repository.run_water_cycle, 'cron', 
                            hour=irrigation_time.hour, minute=irrigation_time.minute) for irrigation_time in irrigation_schedule]

        return irrigation_jobs

    def create_air_jobs(self, air_schedule):
        air_on_jobs = [
            self.scheduler.add_job(actuator_repository.fans_on, 'cron', hour=air_on_time.hour, minute=air_on_time.minute)
            for air_on_time, air_off_time in air_schedule
            ]
        air_off_jobs = [
            self.scheduler.add_job(actuator_repository.fans_off, 'cron',hour=air_off_time.hour, minute=air_off_time.minute) 
            for air_on_time, air_off_time in air_schedule
            ]
    ...
    def create_lighting_jobs(self, lighting_schedule):
        lighting_on_jobs = [self.scheduler.add_job(actuator_repository.lights_on, 'cron',
                     hour=lighting_on_time.hour, minute=lighting_on_time.minute) for lighting_on_time, lighting_off_time in lighting_schedule]
        lighting_off_jobs = [self.scheduler.add_job(actuator_repository.lights_off, 'cron',
                     hour=lighting_off_time.hour, minute=lighting_off_time.minute) for lighting_on_time, lighting_off_time in lighting_schedule]
...
if __name__ == "__main__":
    scheduler_v2 = SchedulerV2() # define scheduler globally
    scheduler_v2.run()