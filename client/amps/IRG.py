...
class Irrigation():
    ...
    def __init__(self, gpioIRGMainPump, gpioIRGWtrSol, gpioIRGTankSwitchSol, gpioIRGNutrSol,
                 gpioIRGlvl5Sol, gpioIRGlvl4Sol, gpioIRGlvl3Sol, gpioIRGlvl2Sol, gpioIRGlvl1Sol,
        ...
        self.IRGlvlSols = [self.IRGlvl5Sol, self.IRGlvl4Sol,
                           self.IRGlvl3Sol, self.IRGlvl2Sol, self.IRGlvl1Sol]
        self.IRGMainTankSensorFull = DigitalInputDevice(
            gpioIRGMainTankSensorFull, pull_up=True)
        self.IRGMainTankSensorEmpty = DigitalInputDevice(
            gpioIRGMainTankSensorEmpty, pull_up=True)
        self.IRGDrainTankSensorFull = DigitalInputDevice(
            gpioIRGDrainTankSensorFull, pull_up=True)
        self.IRGDrainTankSensorEmpty = DigitalInputDevice(
            gpioIRGDrainTankSensorEmpty, pull_up=True)
        self.LEDObj = LEDObj
    ...
    def waterCycle(self, cycleTime=5):
        ...
        # Turning on the main pump and leave open for cycletime
        self.IRGMainPump.on()
    ...
    def supplyTankCheck(self):
        if self.IRGMainTankSensorEmpty.is_active == False:
            # ###Defining variables to test the emailer class
            #sendTo = ['matt.gindlesparger@gmail.com','jeffersonaellinger@gmail.com']
            #emailSubject = "FABSSHOP001 SUPPLY TANK EMPTY"
            #emailContent = "Supply tank is empty. Please refill the watering tank immediately."
            
            # #Sends an email to the "sendTo" address with the specified "emailSubject" as the subject and "emailContent" as the email content.
            #sendEmptyTankMsg = amps.mailer.Emailer()
            #sendEmptyTankMsg.sendmail(sendTo, emailSubject, emailContent)
            print("TANK IS EMPTY")
            #self.panicMode(self.LEDObj)
...
''' Add an indicator on dashoboard tanks levels are full or empty. '''
